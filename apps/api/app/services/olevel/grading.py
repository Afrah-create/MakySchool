...
async def build_enrollment_data(
    ...
    rows = await conn.fetch(
        ...
        LEFT JOIN olevel_exam_sessions es
          ON es.school_id = $2
         AND es.class_id = $3
         AND es.term_id = $4
         AND es.academic_year_id = $5
        LEFT JOIN curriculum_assessment_categories c
          ON c.id = es.category_id
        LEFT JOIN olevel_marks m
          ON m.exam_session_id = es.id
         AND m.student_id = $6
    ...
    for x in rows:
        ...
        if x["category_code"]:
            d["category_scores_raw"][x["category_code"]] = x["raw_score"]
            d["max_marks_by_category"][x["category_code"]] = x["max_marks"]
...
async def persist_result(
    ...
    t = result["totals"]
    # Re-grading clears prior approval so admins must re-approve.
    await conn.execute(
        """
    ...
    *,
    ...
    for x in ids:
        try:
            ...
            calculated += 1
        except Exception as exc:  # noqa: BLE001 — continue grading remaining students
...
async def generate_class_results(
    ...
    *,
    ...
    """Grade every enrolled student, then recalculate class/subject rankings."""
