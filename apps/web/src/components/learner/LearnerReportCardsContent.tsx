...
import {
  ...
  useLearnerPrimaryApprovedReports,
} from '@/hooks/useLearnerPrimary';
...
function ALevelDetail({ report }: { report: ALevelReportCard }) {
  return (
    <>
      <section className="overflow-hidden rounded-xl border border-theme bg-theme-surface">
        <div className="border-b border-theme px-4 py-4 sm:px-5">
          ...
          <dl className="mt-4 grid grid-cols-2 gap-x-4 gap-y-3 border-t border-theme pt-4 sm:grid-cols-4">
            ...
            <MetaChip label="Combination" value={report.combinationName} />
            <MetaChip label="Exam" value={report.examName} />
            <MetaChip
              label="Position"
              value={
        ...
        <div className="grid grid-cols-3 gap-3 border-b border-theme px-4 py-4 sm:px-5">
          <KpiTile label="Principal passes" value={report.principal_pass_count} />
          <KpiTile
            label="Class rank"
            value={
              report.position != null && report.classSize
          ...
          />
          <KpiTile label="Term" value={report.termName || '—'} />
        </div>
            ...
            <tbody>
              {report.subjects.map((s) => (
                <tr key={s.subjectId}>
                  ...
                  <td className="text-muted">{s.descriptor || '—'}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <CommentsBlock
        ...
        headTeacher={report.headTeacherComment}
      />
    </>
  );
}

function PrimaryDetail({ report }: { report: PrimaryReportCard }) {
  const name = report.studentName || report.student.fullName;
      ...
      <section className="overflow-hidden rounded-xl border border-theme bg-theme-surface">
        <div className="border-b border-theme px-4 py-4 sm:px-5">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div className="flex min-w-0 items-start gap-3.5">
              <ReportAvatar
                photoUrl={report.photoUrl ?? report.student.photoUrl}
                initials={report.studentInitials}
                name={name}
              />
              <div className="min-w-0">
                ...
                <p className="mt-0.5 font-mono text-xs text-theme-muted">
                  {report.learnerId ?? report.student.learnerId}
                </p>
              </div>
            </div>
            <div className="flex shrink-0 items-center gap-3 self-start rounded-xl border border-theme bg-theme-raised/40 px-4 py-2.5 sm:flex-col sm:items-end sm:gap-1">
              ...
              <span className="inline-flex rounded-full bg-theme-raised px-2.5 py-0.5 text-xs font-semibold text-theme-muted">
                {totals?.division || totals?.overallGrade || '—'}
              </span>
            </div>
          </div>
          <dl className="mt-4 grid grid-cols-2 gap-x-4 gap-y-3 border-t border-theme pt-4 sm:grid-cols-4">
            <MetaChip
              label="Class"
              value={report.className ?? report.student.className}
            />
            <MetaChip
              label="Exam"
              value={[report.examName, report.examTypeName]
                .filter(Boolean)
                .join(' · ')}
            />
...
export function LearnerReportCardsContent() {
  ...
  const items = useMemo((): UnifiedSummary[] => {
    ...
    for (const r of alevelList.data ?? []) {
      out.push({
        key: `alevel:${r.examId}`,
        track: 'alevel',
        examId: r.examId,
        ...
        subtitle: `${r.total_points} pts · ${RESULT_LABEL[r.result_code] ?? r.result_code}`,
      });
    }
  ...
  const alevelDetail = useLearnerApprovedReport(
    selected?.track === 'alevel' ? selected.examId : '',
    selected?.track === 'alevel',
  );
  const primaryDetail = useLearnerPrimaryApprovedReport(
  ...
  return (
    <DashboardPage
      ...
      title="Report cards"
      description="View and download approved examination reports."
      actions={
        selected ? (
          <LoadingButton
            variant="primary"
    ...
    >
      {listLoading ? (
        ...
        <EmptyState
          ...
          description="When your school approves a report card, it will appear here for download."
        />
      ) : (
          ...
          <aside className="overflow-hidden rounded-xl border border-theme bg-theme-surface lg:sticky lg:top-4 lg:max-h-[calc(100vh-6rem)] lg:flex lg:flex-col">
            ...
            <ul className="hidden min-h-0 flex-1 overflow-y-auto lg:block">
              {items.map((item) => {
                ...
                return (
                  <li key={item.key} className="border-b border-theme last:border-b-0">
                    <button
                      type="button"
                      ...
                      className={[
                        ...
                        active
                          ...
                          : 'border-l-2 border-transparent text-theme-primary hover:bg-theme-raised/40',
                      ].join(' ')}
                    >
