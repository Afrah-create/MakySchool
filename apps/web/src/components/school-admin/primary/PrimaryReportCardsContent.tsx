...
import { useResourceTerms } from "@/hooks/useResources";
import {
  useBulkSavePrimaryReportComments,
  useGeneratePrimaryReportCards,
  usePrimaryClasses,
...
function PrimaryReportCardsClient() {
  ...
  useEffect(() => {
    if (!termId) {
      ...
      else if (terms[0]) setTermId(terms[0].id);
    }
  }, [terms, termId]);
  ...
  const saveComment = useSavePrimaryReportComment();
  ...
  if (!offers) {
    return (
      <div className="mx-auto max-w-7xl space-y-6 p-4 sm:p-6">
        <PageHeader title="Primary report cards" />
        <EmptyState
  ...
  const studentName =
    report?.studentName || report?.student?.fullName || "Learner";
  const learnerId = report?.learnerId ?? report?.student?.learnerId;
  const photoUrl = report?.photoUrl ?? report?.student?.photoUrl;
    ...
    <div className="mx-auto max-w-7xl space-y-6 p-4 sm:p-6">
      ...
      {classes.length === 0 ? (
            ...
            <EmptyState
              ...
              action={
                <Link href="/dashboard/primary/exams" className="ms-btn-primary">
                  Manage exams
                </Link>
              }
            />
                    ...
                    >
                      {students.map((s) => (
                        <option key={s.studentId} value={s.studentId}>
                          #{s.classPosition ?? "—"} · {s.studentName}
                          {s.aggregate != null ? ` (Agg ${s.aggregate})` : ""}
                        </option>
                      ))}
                    </select>
                  </label>
                      ...
                      >
                        ...
                        >
                          <span className="block truncate text-sm font-medium">
                            {s.studentName}
                            {s.approvedAt ? (
                              <span className="ml-1 text-[10px] font-semibold text-theme-success">
                                ✓
                              </span>
                            ) : null}
                          </span>
                    ...
                    <section className="overflow-hidden rounded-xl border border-theme bg-theme-surface">
                            ...
                            <div className="min-w-0">
                              ...
                              {approved && report.approvedByName ? (
                                <p className="mt-1 text-xs text-theme-success">
                                  Approved by {report.approvedByName} · visible
                                  to learner
                                </p>
                              ) : null}
                            </div>
                      ...
                      <div className="grid grid-cols-3 gap-3 border-b border-theme px-4 py-4 sm:px-5">
                        <KpiTile
                          ...
                          value={
                            totals?.averagePercent != null
                              ? `${totals.averagePercent.toFixed(1)}%`
                              : null
                          }
                        />
                        <KpiTile label="Grade" value={totals?.overallGrade} />
                        <KpiTile
                          label="Aggregate"
                          value={totals?.aggregate ?? totals?.division}
                        />
                      </div>
