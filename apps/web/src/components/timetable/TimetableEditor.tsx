...
import { can, formatClassLabel } from "@makyschool/shared/constants";
import type {
  ClassWithDetails,
...
import { AcademicTabNav } from "@/components/school-admin/academic/AcademicLayout";
...
type TimetableTab = "builder" | "periods";

export function TimetableEditor() {
  const role = useCurrentRole();
  const canManage = Boolean(role && can(role, "manageTimetable"));
  ...
  const [pickedTab, setPickedTab] = useState<TimetableTab | null>(null);
  ...
  // The builder is unusable without a bell schedule, so open on that tab instead.
  const preferredTab: TimetableTab =
    pickedTab ?? (periodTemplates && periodTemplates.length === 0 ? "periods" : "builder");
  const activeTab: TimetableTab = canManage ? preferredTab : "builder";
  ...
  return (
    <div className="mx-auto w-full max-w-[1600px] space-y-5">
      <header className="flex flex-col gap-4 border-b border-theme pb-4 lg:flex-row lg:items-end lg:justify-between">
      ...
      {canManage ? (
        <AcademicTabNav<TimetableTab>
          tabs={[
            { id: "builder", label: "Class timetable" },
            { id: "periods", label: "Teaching periods", count: templates.length },
          ]}
          active={activeTab}
      ...
      {activeTab === "periods" ? (
        ...
        <div className="space-y-5">
          <div className="ms-card flex flex-col gap-3 p-4 sm:flex-row sm:items-end sm:justify-between sm:p-5">
            ...
            <dl className="flex flex-wrap gap-x-6 gap-y-2 text-xs sm:justify-end">
              <div>
                <dt className="font-medium uppercase tracking-wide text-theme-muted">Periods</dt>
                <dd className="mt-0.5 text-sm font-medium text-theme-primary">
                  {templates.length || "—"}
                </dd>
              </div>
              <div>
                <dt className="font-medium uppercase tracking-wide text-theme-muted">Day span</dt>
                <dd className="mt-0.5 text-sm font-medium text-theme-primary">
                  {scheduleSpan ?? "—"}
                </dd>
              </div>
              <div>
                <dt className="font-medium uppercase tracking-wide text-theme-muted">Slots filled</dt>
                <dd className="mt-0.5 text-sm font-medium text-theme-primary">
                  {draft.size}
                </dd>
              </div>
              ...
              action={
                canManage ? (
                  <button
                    ...
                    onClick={() => setPickedTab("periods")}
                  >
                    Go to teaching periods
                  </button>
                ) : null
              }
            />
